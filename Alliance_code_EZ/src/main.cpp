...
ez::Drive chassis(
    ...
    {-10, -9},     // Left Chassis Ports (negative port will reverse it!)
...
void opcontrol() {
  ...
  while (true) {
    ...
    // conveyer
    Conveyer((master.get_digital(DIGITAL_R1) - master.get_digital(DIGITAL_R2))*200);

    // lady brown
    Ladybrown(master.get_digital(DIGITAL_UP)*200 - master.get_digital(DIGITAL_DOWN)*200);
    ...
    // corner pneumatics
    Corner(master.get_digital(DIGITAL_A));
